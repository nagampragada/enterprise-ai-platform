...
`GET /api/v1/health` is public strict readiness. Success is HTTP 200 only when runtime configuration, bounded database connectivity, an explicitly compatible Alembic revision, and strict-runtime GitHub/Secret Manager composition are ready. During the Phase 3 Slice 3 expand-migration transition, the exact compatible revisions are `20260831_000021` and `20260902_000022`; no other older, newer, malformed, missing, or branched revision is accepted. The fixed response distinguishes Boolean `schema_compatible`, `schema_current`, and `migration_required` values while retaining the `schema` ready/not-ready check. Revision `20260831_000021` is compatible but not current and requires migration; `20260902_000022` is compatible and current. Any failed check returns HTTP 503. The contract never returns an exception, database URL, SQL, project identifier, secret reference, credential, or provider detail, and never retrieves secret values or contacts GitHub/OpenAI.
