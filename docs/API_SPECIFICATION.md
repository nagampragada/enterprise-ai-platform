...
The established scope-specific form
`POST /api/v1/connectors/{connector_id}/scopes/{scope_id}/sync-jobs` normally
accepts `{}`. For a reviewed controlled GitHub run it may instead accept one
`reservation` object containing a canonical reservation UUID, a write-only
canonical 32-byte base64url owner capability generated by a cryptographically
secure random source equivalent to `secrets.token_bytes(32)`, an integer
lifetime from 300 to 7,200 seconds, and exact lowercase source-hash/blob/
revision/profile fields.
The authenticated administrator is the reservation creator; no tenant or user
identity is client supplied. Reservation and job commit atomically. Responses
never return the capability, its digest, or reservation lifecycle data. A
reservation is rejected for Local Folder scopes or if coalescing cannot prove
the exact same live reservation. An ordinary request also cannot coalesce onto
a live reserved job. This authenticated public route creates job/reservation
control-plane state only; it does not execute a planner, processor, or provider
operation. The database hash protects durable storage, not CLI/environment/
execution-metadata transport of the raw bearer capability.
...
`GET /api/v1/health` is public strict readiness. Success is HTTP 200 only when runtime configuration, bounded database connectivity, an explicitly compatible Alembic revision, and strict-runtime GitHub/Secret Manager composition are ready. During the controlled-reservation expand transition, the exact compatible revisions are `20260905_000024` and `20260911_000025`; no other older, newer, malformed, missing, or branched revision is accepted. The fixed response distinguishes Boolean `schema_compatible`, `schema_current`, and `migration_required` values while retaining the `schema` ready/not-ready check. Revision `20260905_000024` is compatible but not current and requires migration; `20260911_000025` is compatible and current. Any failed check returns HTTP 503. The contract never returns an exception, database URL, SQL, project identifier, secret reference, credential, or provider detail, and never retrieves secret values or contacts GitHub/OpenAI.
