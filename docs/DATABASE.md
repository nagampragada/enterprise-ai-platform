...
This slice provides persistence only. No execution-control repository, worker, scheduler, background recovery process, or API exists or runs yet, and the existing Local Folder service is not changed to enqueue or lease jobs. Future repositories and services own atomic polling/claiming, generation increments, fencing predicates, heartbeats, cancellation races, retry policy, maximum-attempt transitions, run allocation, commits/rollbacks, and audit events. Regular employees cannot configure Local Folder connectors through this slice.
...
Current limitations: Local Folder rename is delete-plus-create, supported extensions are `.txt`, `.md`, `.markdown`, `.docx`, and `.pdf`, and provider-native incremental deltas, work leasing, scheduling, APIs, automatic abandoned-run recovery, and secret management remain future work.
