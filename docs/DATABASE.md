...
The internal GitHub content reader also requires no migration. It treats the selected scope as the authorization boundary, copies validated immutable identifiers during a short read transaction, and requires that transaction to end before SecretStore or GitHub access. Repository snapshots, commit/tree/blob identities, raw bytes, SHA-256 results, tokens, provider responses, and traversal state remain request memory only. Before a future synchronization stage persists a `SourceItem` or document/version observation, it must begin a new transaction and revalidate the connector and scope boundary; that persistence stage is not implemented here.
