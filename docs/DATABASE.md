...
`trigger_type` records the original safe request provenance: `manual`, `scheduled`, `webhook`, or `system`. Retry and recovery are derived execution events represented by higher job attempt/fencing values and retry runs, not rewritten original trigger provenance. Optional requesting-user attribution is tenant-safe and clears only that column when the user is deleted. This persistence slice does not add schedules; a future scheduler may enqueue a scheduled job with its first eligibility time.
...
Repositories and application services retain caller-owned transactions. `LocalFolderSyncWorker` is the transaction-boundary adapter: it opens and closes short-lived sessions, commits acquisition plus linked-run allocation before folder access, renews heartbeats independently, and executes at most one continuation step by default with a hard limit of ten. Incomplete discovery or reconciliation keeps the same job, run, attempt, lease, and fence without consuming a retry. Immediately before progress commit, a conditional heartbeat update revalidates tenant, worker, lease, fence, attempt, running state, expiry, and cancellation while taking the job-row lock; failure rolls back all database progress from that step. Final Local Folder progress, run completion, and job success commit atomically.

Cancellation is checked before folder access, before each bounded step, between steps, at the progress fence barrier, and in the conditional success update. One indivisible filesystem, extractor, or embedding call cannot be interrupted and cancellation is observed at the next safe boundary. Extraction and embedding currently occur inside the bounded continuation transaction, so source/run/version/indexing locks may remain held for that one batch, never for the whole folder. A process can crash after an embedding provider responds but before PostgreSQL commits; without provider idempotency the external call may repeat after recovery. Committed source/version/indexing state prevents repeating successful committed work, but provider calls are at-least-once rather than exactly-once.

No continuously running daemon, scheduler, polling loop, automatic recovery process, or API exists yet; callers invoke the bounded runner once. Audit event emission also remains a future application responsibility. Organization-wide daily token and embedding-cost budgets, a provider-wide durable circuit breaker, and manual financial approvals require separate tenant usage/budget persistence and are deliberately not simulated with process memory or constants. Regular employees cannot configure Local Folder connectors through this slice.

#### Local Folder synchronization orchestration

`LocalFolderSynchronizationService` runs one caller-committed bounded batch for an active `local_folder` connector and active `platform_managed` folder scope. The scope's persisted absolute `external_scope_key` is the only filesystem root; requests cannot override it. Roots must exist as ordinary directories, explicit traversal segments and root symlinks are rejected, discovered symlinks are skipped, and every content path is resolved again through the Local Folder connector's containment checks. Public results, errors, run metadata, source metadata, and indexing summaries contain no absolute paths or source content.

Discovery is lazy and deterministic by case-sensitive root-relative POSIX identity. Persisted sync cursors record only the run-owned phase and last processed relative key or membership cursor. A running sync may therefore span caller commits without one unbounded transaction. Local Folder performs full checksum discovery rather than provider delta discovery: unchanged checksums reuse the current immutable version and skip indexing; new or changed content uses repository-serialized version allocation, deterministic extraction/chunking/embedding profile identity, append-oriented indexing attempts, and moves materialization only after successful indexing.

Complete discovery is required before missing-item reconciliation. Reconciliation pages only active memberships in the current scope whose `last_seen_at` predates the run start. Removing one scope membership never removes another; the canonical source becomes unavailable only when no active membership remains. Immutable versions, indexing history, documents, and chunks are retained.

The connector row is locked for each batch, source/version/state locks remain held until caller transaction completion, and PostgreSQL permits only one active run per scope. The service creates no sessions and never commits, rolls back, or retries. On extraction, embedding, or persistence failure it records only safe failure state in the current transaction and raises; callers must roll back, preserving the previous current version, chunks, and materialization. Retrying starts or continues from persisted run progress without duplicating completed source items, memberships, versions, or attempts.
