...
The internal GitHub content reader and staged synchronization service also require no migration. The selected scope is the authorization boundary. A short read transaction copies validated immutable identifiers and must end before SecretStore, GitHub, extraction, chunking, or embedding access. The synchronization service persists only safe provider identities and platform checksums through the existing source/version/materialization/indexing/chunk schema; tokens, raw bytes, provider responses, and extracted payloads never enter provider metadata or cursor state.

GitHub traversal uses the existing `connector_sync_cursors.safe_cursor` JSONB object with cursor type `github_repository_progress`. Schema version 1 contains a pinned repository ID/default-branch/commit/root-tree snapshot, a bounded iterative DFS frame array, cumulative entry/file/byte/chunk totals, and `scan_complete`. Service validation imposes a fixed field allowlist, maximum depth 64, lowercase SHA-1/SHA-256 object IDs, safe repository-relative paths, finite nonnegative totals, and a 96 KiB serialized maximum. The cursor is owned by the existing organization/connector/scope/run foreign keys and contains no lease values or secrets. No new column, constraint, index, or migration is required.
...
Batch persistence revalidates the complete job lease/fence and active GitHub authorization, locks the active cursor and canonical source state, and writes source/membership, version/materialization, indexing attempt/state, document/chunks, sync item/counters, and the next cursor in one caller-owned transaction. Preparation occurs before that transaction. Cursor failure therefore rolls back file state and file failure cannot advance the cursor. Completion marks the run/job successful but performs no membership deletion reconciliation.
...
- Integration status: connector instance/scope repositories and Local Folder/GitHub management services/APIs exist. GitHub repository scopes persist desired boundaries, and an internal staged service persists bounded creates/updates; GitHub worker routing and deletion reconciliation are not implemented.
