...
Transactions remain caller-owned: claim and run allocation in a short transaction, commit before expensive work, execute one attempt without a database row lock, validate the same fence in progress transactions, then finalize in another short transaction. Repositories and services do not create sessions, commit, roll back, or retry transactions internally. The Local Folder service is unchanged; cancellation can only be observed at future bounded worker continuation points and no mid-file cancellation is claimed.

No continuously running worker, scheduler, polling loop, recovery daemon, or API exists yet. Audit event emission also remains a future application responsibility. Organization-wide daily token and embedding-cost budgets, a provider-wide durable circuit breaker, and manual financial approvals require separate tenant usage/budget persistence and are deliberately not simulated with process memory or constants. Regular employees cannot configure Local Folder connectors through this slice.
...
Current limitations: Local Folder rename is delete-plus-create, supported extensions are `.txt`, `.md`, `.markdown`, `.docx`, and `.pdf`, and provider-native incremental deltas, worker integration, scheduling, APIs, automatic recovery invocation, and secret management remain future work.
