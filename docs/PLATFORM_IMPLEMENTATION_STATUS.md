...
`ConnectorManagementService` and authenticated APIs support Local Folder and GitHub connector creation/list/get; Local Folder additionally supports scope creation/list and synchronization operations. GitHub connectors declare dedicated discovery and selection capabilities while public content download/synchronization, ACL, folder, deletion, history, and webhook capabilities remain false. Organization-admin-only operations manage App installation, discover a live page, and create/list/idempotently remove repository scopes. Internal provider/application boundaries resolve a selected repository snapshot, traverse bounded non-recursive trees, download supported blobs, and stage source/document/version/index/chunk persistence, but no route exposes them and no production host claims GitHub jobs. Responses omit `safe_config`, secret references, credentials, provider payloads, and ORM state. Connector update/archive, cancellation, GitHub worker routing, and UI remain absent.
