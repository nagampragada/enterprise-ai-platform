...
Implemented runtime exposure includes connector/scope creation and reads, interval schedule create/read/pause/resume/delete, provider-neutral manual job enqueue, bounded connector job listing/detail/run summaries, cooperative cancellation, continuous/one-shot scheduling, Local Folder and GitHub worker routing, and expired-lease recovery. Cron/timezone schedules, ACL synchronization, retrieval APIs, LangGraph, LangChain, UI, additional cloud connectors, and answer generation remain absent.
...
| GitHub | Verified App installation, live discovery, tenant-safe selection, bounded content reading, staged create/update/retirement/reconciliation, production worker routing, and provider-neutral job operations implemented |
...
`ConnectorManagementService` and authenticated APIs support Local Folder and GitHub connector creation/list/get plus provider-neutral manual job enqueue, bounded connector job listing/detail/run summaries, and cancellation. GitHub connectors declare dedicated discovery and selection capabilities while ACL, folder, history, and webhook capabilities remain false. Organization-admin-only operations manage App installation, discover a live page, and create/list/idempotently remove repository scopes. Internal provider/application boundaries resolve a selected repository snapshot, traverse bounded non-recursive trees, download supported blobs, and stage source/document/version/index/chunk persistence; the production connector host claims both Local Folder and GitHub jobs. Responses omit `safe_config`, secret references, credentials, provider payloads, worker leases/fences, raw exceptions, and ORM state. Connector update/archive, item/error recovery controls, audit emission, and UI remain absent.
...
### Internal GitHub staged synchronization
...
- Leave webhooks disabled/not configured. Bounded content reading, staged create/update/retirement logic, production GitHub worker routing, independent heartbeat orchestration, and public job operations are implemented; ACL synchronization, branch-history import, folders, and webhooks are not operational.
...
| Connector administration API breadth | Create/read/enqueue/list/detail/bounded runs/cancel implemented; update/archive/credentials/audit and item/error recovery controls remain gaps |
...
| GitHub content sync/webhooks/ACL sync | Continuous worker routing, bounded staged synchronization/reconciliation, operational job APIs, cancellation, and independent heartbeat exist; deployment, webhooks, and ACL sync remain future work |
...
- [x] Provider-neutral manual job, connector history, bounded run summary, and cancellation APIs
- [ ] Connector update/archive/credential and detailed item/error recovery APIs
