...
| Snapshot commit | `83fa4560f916f8fc236b7f53359ea9f26bdab0bf` (clean implementation baseline) |
...
Operationally today, the FastAPI backend supports authentication and authenticated manual upload of TXT, Markdown, DOCX, and PDF files through a fully tested extraction, deterministic chunking, embedding, and PostgreSQL/pgvector persistence pipeline. The backend also contains secure Local Folder connector-management APIs, recurring interval schedules, a database-only scheduler host, a complete synchronization engine, durable job control, a continuous and one-shot worker host, immutable source versioning, and a permission-aware vector retrieval repository. GitHub is the first cloud connector: organization administrators can complete its GitHub App installation lifecycle, discover a bounded live repository page, and persist explicit repository-to-knowledge-space selections after authoritative restricted-token revalidation. An internal read-only foundation now resolves immutable default-branch snapshots, validates one non-recursive Git tree, and streams one bounded blob entirely in memory; it has no public API and creates no source/document/index state. A production Google Cloud Secret Manager adapter supplies immutable GitHub configuration references, ephemeral PKCE storage, and on-demand private-key access through ADC-based, fail-closed composition. No GCP resource or Cloud Run deployment has been provisioned. GitHub repository synchronization is not implemented. Retrieval is not exposed by an API.
...
`ConnectorManagementService` and authenticated APIs support Local Folder and GitHub connector creation/list/get; Local Folder additionally supports scope creation/list and synchronization operations. GitHub connectors declare dedicated discovery and selection capabilities while public content download/synchronization, ACL, folder, deletion, history, and webhook capabilities remain false. Organization-admin-only operations manage App installation, discover a live page, and create/list/idempotently remove repository scopes. An internal provider/application boundary can resolve a selected repository snapshot, list one tree, and download one supported blob, but no route exposes it and it never starts synchronization. Responses omit `safe_config`, secret references, credentials, provider payloads, and ORM state. Connector update/archive, cancellation, GitHub content synchronization, and UI remain absent.
...
Persisted installation data is limited to the untrusted positive candidate installation ID and setup timestamp, followed after verification by safe binding and credential metadata. Repository discovery results remain live and unpersisted; explicit repository choices persist only the canonical repository ID and a fixed safe metadata allowlist in `ConnectorScope`. OAuth state is stored only as a SHA-256 digest. Temporary user tokens, callback codes, App JWTs, discovery tokens, and repository-restricted selection tokens are discarded after immediate use. No credential, raw provider response, authorization header, browser-supplied repository metadata, or arbitrary redirect is persisted or returned.
...
`POST /api/v1/connectors/{connector_id}/github/repository-scopes` accepts only a positive repository ID and tenant-owned knowledge-space UUID. It validates persisted boundaries, ends that transaction, creates one token restricted to exactly that repository with Metadata read only, and requires both the token response and a one-item restricted repository listing to agree. A second transaction re-locks and revalidates every boundary before storing `github:repository:{id}` in `ConnectorScope`. Existing connector/external-key uniqueness and connector locking provide idempotency and concurrency safety without a migration or implicit knowledge-space movement.

GET returns only a bounded deterministic page of persisted safe selections and never calls GitHub. DELETE is an idempotent provider-free local `removed` transition; it neither hard-deletes history nor changes remote App access. No route creates source items, documents, chunks, indexes, jobs, runs, or schedules.
...
This capability is backend-internal: OpenAPI is unchanged, and there is no recursive crawl, branch/ref selector, source/document/version/chunk persistence, extraction, embedding, indexing, job, cursor, webhook, ACL sync, clone, archive, temporary file, or provider-token persistence. Planned stable source identity is `github:repository:{repository_id}:path:{canonical_repository_path}`; immutable version evidence will combine commit ID, blob ID, platform SHA-256, and exact size. The next GitHub slice is staged synchronization using the existing job/lease infrastructure.
...
| POST | `/api/v1/connectors/{connector_id}/github/repository-scopes` | Bearer + `organization_admin` | Select one verified repository into one active tenant space | repository ID + knowledge-space UUID only | immutable safe repository scope | Complete |
| GET | `/api/v1/connectors/{connector_id}/github/repository-scopes` | Bearer + `organization_admin` | Provider-free persisted selection page | bounded limit/cursor | immutable safe repository scope page | Complete |
| DELETE | `/api/v1/connectors/{connector_id}/github/repository-scopes/{scope_id}` | Bearer + `organization_admin` | Idempotent local deselection | UUID paths | removed safe repository scope | Complete |
...
Result: **1227 passed, 1 skipped, 55 warnings; exit code 0**.
...
`configure_mappers()` passed during this snapshot validation; migration tests also reflect ORM/schema parity.
...
| GitHub content sync/webhooks/ACL sync | Internal immutable snapshot/tree/bounded-blob foundation exists; staged persistence, webhooks, and ACL sync remain future work |
...
| 10 | GitHub staged read-only synchronization | Content identity/download foundation complete; next add bounded job-driven persistence, then webhooks and ACL synchronization | GPT-5.6 Sol, high | GitHub App permission review required |
