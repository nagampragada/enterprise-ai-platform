...
There is no public snapshot, tree, blob, or file-download route. The production synchronization worker uses the internal content reader only through a staged boundary: a short tenant-qualified database read validates the active GitHub connector, active repository scope, canonical `github:repository:{repository_id}` key, strict safe scope configuration, active knowledge space, active App-installation credential, connected verified organization installation, owner/account agreement, and recorded `Metadata: read` plus `Contents: read` grants. It copies immutable identifiers into a redacted frozen authorization object. The caller must end the database transaction before any SecretStore or GitHub call; the service rejects provider access while its injected session has an open transaction. Every later operation repeats this short authorization step, and each persistence stage revalidates the connector/scope boundary before writing.
...
## Packaged sandbox runtime boundary

The common backend image runs the API through `python -m app.server`, the connector worker through `python -m infrastructure.workers.connector_sync_worker_host --once`, and the scheduler through `python -m infrastructure.workers.connector_sync_scheduler_host --once`. Legitimate worker no-work and scheduler no-due-work results exit zero; startup, database, and processing failures remain nonzero.

The sandbox API is internet reachable for GitHub setup and callback delivery. Administrator routes retain application JWT and role enforcement. Setup/callback safety relies on hashed state, PKCE, expiry, single use, and authoritative GitHub verification. Worker and scheduler have no public HTTP endpoint. The worker identity needs database, OpenAI, and only the pinned private-key secret; OAuth client-secret access belongs to the API identity. Exact placeholder deployment steps are in `GCP_GITHUB_SANDBOX_RUNBOOK.md`. No deployment has occurred.
