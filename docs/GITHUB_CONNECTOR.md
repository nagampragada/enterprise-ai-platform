...
The GitHub connector implements the secure GitHub App installation lifecycle, live read-only repository discovery, explicit tenant-safe repository selection, the internal content reader, and an internal bounded staged synchronization service. The synchronization service can create and update indexed content for one selected repository, but it is not yet routed by the production worker host. Webhooks, deletion reconciliation, ACL synchronization, retrieval APIs, and frontend integration remain unimplemented.
...
The stable source identity is `github:repository:{repository_id}:path:{canonical_repository_path}`. Repository ID—not mutable owner/name—is the security identity, so a repository rename does not change repository identity. Until rename reconciliation exists, a path rename is create-plus-unreconciled-old-path. Synchronization records the blob object ID as provider revision, the pinned commit ID as snapshot evidence, and platform SHA-256 plus exact size as content evidence. The low-level reader itself remains read-only; persistence is owned by the staged synchronization service below.

## Internal staged repository synchronization

`GitHubStagedSynchronizationService` and `GitHubSynchronizationPreparationService` form an internal, caller-transaction-owned pipeline. They are not public APIs and are not registered in the Local Folder production worker host. A caller first validates the job/run lease and captures a redacted repository authorization object in a short transaction, closes that transaction, resolves the persisted default branch to an immutable commit/root-tree pair, then pins that pair in a run-owned cursor. Every later continuation reuses those object IDs even if the branch moves.

Traversal is iterative depth-first search over non-recursive Git Trees requests. Cursor schema version 1 stores only the snapshot identity, at most 64 parent frames (`tree_path`, `tree_object_id`, `next_entry_index`), finite cumulative counters, and a completion flag. The cursor uses a fixed field allowlist, lowercase 40/64-character object IDs, validated relative paths, a 96 KiB serialized cap, and no token, URL, secret, raw response, or content. Corrupt, incompatible, oversized, backward, cross-snapshot, or cross-repository progress fails closed. Re-fetching a frame always addresses an immutable tree ID.

Supported extensions are `.pdf`, `.docx`, `.txt`, `.md`, and `.markdown`; comparison is case-insensitive. Unsupported extensions are advanced without download. Symlinks and submodules never reach traversal because the content reader omits them, and Git LFS is rejected by bounded blob reading. Missing-size, over-10-MiB, or identity-too-long supported files are recorded as skipped rather than indexed.
...
One short fenced persistence transaction revalidates the lease owner, lease UUID, fencing attempt, cancellation/expiry, active connector/scope/credential/installation/knowledge-space authorization, and pinned cursor. It then creates or updates `SourceItem` and membership, allocates one immutable current version as needed, maintains the document materialization, indexing state/attempt, and pgvector chunks, completes sync items/counters, and advances the cursor atomically. A failed file or cursor write rolls the batch back; a stale prepared batch cannot write. Full traversal records `scan_complete` and completes the run/job without removing unseen items.
...
This slice deliberately provides no deletion authority: an incomplete or complete scan never removes unseen prior items. Repository/path rename reconciliation, GitHub worker-host claiming/routing and heartbeat choreography, webhooks, ACLs, arbitrary refs, Git LFS, clone/archive access, and public synchronization/content APIs remain separate work. The next slice is deletion reconciliation plus production worker routing.

Connector capability output now explicitly reports `supports_repository_discovery`, `supports_repository_selection`, `supports_bounded_content_reading`, and `supports_staged_synchronization` as true for GitHub. `supports_incremental_sync`, deletions, ACL/permissions, webhooks, and version-history import remain false. “Staged synchronization” describes the internal create/update service, not automatic worker execution.
...
The next GitHub slice is deletion reconciliation plus production worker routing for the staged synchronization service. Recursive whole-repository requests remain intentionally absent. Webhooks, ACLs, rename reconciliation, search, and answers remain later work.
