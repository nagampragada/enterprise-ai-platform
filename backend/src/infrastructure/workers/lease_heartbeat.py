...
class FileWorkLeaseHeartbeat:
    """Independent renewal for one fenced connector file-work lease."""
    ...
    def __enter__(self) -> FileWorkLeaseHeartbeat:
        if self._thread is not None:
            raise RuntimeError("heartbeat is already running")
        self._thread = threading.Thread(
            target=self._run,
            name=f"file-work-heartbeat-{self._lease.work_item_id}",
            daemon=False,
        )
        self._thread.start()
        return self
    ...
    def raise_if_failed(self) -> None:
        if self._failure is not None:
            raise LeaseHeartbeatFailure("file-work lease heartbeat stopped") from self._failure
    ...
    def stop(self) -> None:
        ...
        if thread is not None:
            thread.join(self._shutdown_timeout)
            if thread.is_alive():
                raise LeaseHeartbeatFailure("file-work lease heartbeat did not stop")
        self.raise_if_failed()
    ...
    def _run(self) -> None:
        while not self._stop.wait(self._interval):
            session = self._session_factory()
            try:
                ConnectorSyncWorkLedgerRepository(session).heartbeat(
                    self._lease,
                    worker_id=self._worker_id,
